...
namespace Gurux.DLMS.Reader
{
    public class GXDLMSReader
    {
        ...
        /// <summary>
        /// Busqueda de los valores por obisCode
        /// </summary>
        /// <param name="obisCode"></param>
        /// <param name="value"></param>
        public void GetValueByObisCode(string obisCode, out double value)
        {
            value = 0;
            char[] splitSeparator = { '.', ':', '-' };
            var obisObjetive = obisCode.Split(splitSeparator);
            foreach (GXDLMSObject it in Client.Objects)
            {
                var obisLocal = it.LogicalName.Split(splitSeparator);
                if (obisLocal[0] == obisObjetive[0] &&
                    obisLocal[1] == obisObjetive[1] &&
                    obisLocal[2] == obisObjetive[2] &&
                    obisLocal[3] == obisObjetive[3] &&
                    obisLocal[4] == obisObjetive[4])
                {
                    object val3 = Read(it, 3);
                    value = double.Parse(Read(it, 2).ToString());
                    return;
                }
            }
        }
